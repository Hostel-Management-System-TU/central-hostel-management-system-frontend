...
import HomeLoaderSkeliton from "../components/loaders/HomeLoaderSkeliton";
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDashboardDetails({
        ...
        verified_payments: res.data.verified_payments,
        pending_payments: res.data.pending_payments,
        rejected_payments: res.data.rejected_payments,
        pending_reports: res.data.pending_reports,
        resolved_reports: res.data.resolved_reports,
      });
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">
            Welcome back,{" "}
            {user_details ? user_details.name.split(" ")[0] : "User"}!
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="relative">
            ...
            <p className="text-xl font-bold text-slate-800">
              ₹{dashboardDetails.total_paid.toLocaleString("en-IN")}
            </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="relative">
            ...
            <p className="text-xl font-bold text-slate-800">{dashboardDetails.total_payments}</p>
            <p className="text-[10px] text-slate-500 font-medium mt-1 uppercase tracking-wider">
              Payments
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="relative">
            ...
            <p className="text-xl font-bold text-slate-800">{dashboardDetails.pending_reports}</p>
            <p className="text-[10px] text-slate-500 font-medium mt-1 uppercase tracking-wider">
              Pending Reports
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="relative">
            ...
            <p className="text-xl font-bold text-slate-800">
              {dashboardDetails.resolved_reports}
            </p>
            <p className="text-[10px] text-slate-500 font-medium mt-1 uppercase tracking-wider">
              Resolved
            </p>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 rounded-xl bg-emerald-50/50 border border-emerald-100">
                  <p className="text-2xl font-bold text-emerald-700">
                    {dashboardDetails.verified_payments}
                  </p>
                  <p className="text-[10px] text-emerald-600 font-medium uppercase tracking-wider mt-1">
                    Approved
                  </p>
                </div>
                <div className="text-center p-4 rounded-xl bg-amber-50/50 border border-amber-100">
                  <p className="text-2xl font-bold text-amber-700">
                    {dashboardDetails.pending_payments}
                  </p>
                  <p className="text-[10px] text-amber-600 font-medium uppercase tracking-wider mt-1">
                    Pending
                  </p>
                </div>
                <div className="text-center p-4 rounded-xl bg-rose-50/50 border border-rose-100">
                  <p className="text-2xl font-bold text-rose-700">
                    {dashboardDetails.rejected_payments}
                  </p>
                  <p className="text-[10px] text-rose-600 font-medium uppercase tracking-wider mt-1">
                    Rejected
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            {reports.length === 0 ? (
              ...
              <div className="space-y-3">
                {reports.map((report) => {
                  const typeConfig =
                    REPORT_TYPE_MAP[report.report_type] || REPORT_TYPE_MAP[8];
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2.5">
                          <div
                            className={`w-8 h-8 rounded-lg ${typeConfig.bg} ${typeConfig.border} border flex items-center justify-center shrink-0`}
                          >
                            <TypeIcon
